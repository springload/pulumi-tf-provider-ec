PROJECT_NAME := ec Package

SHELL						 := bash
PACK             := ec
ORG              := springload
PROJECT          := github.com/${ORG}/pulumi-tf-provider-${PACK}
...
prepare::
	...
	if [[ "${OS}" != "Darwin" ]]; then \
		sed -i 's,github.com/springload/pulumi-tf-provider-ec,${REPOSITORY},g' provider/go.mod; \
...
build_sdks:: install_plugins provider build_nodejs build_python build_go # build all the sdks
